...
class ReportTestResult(unittest.TestResult):
    ...
    def stopTestRun(self, title=None, url=None, jenkins_time=None, jenkins_branch=None) -> dict:
        """
            所有测试执行完成后, 执行该方法
        :param jenkins_branch: 部署分支
        :param url: 测试网页的url
        :param jenkins_time: 部署时间
        :param title:
        :return:
        """
        ...
        self.fields['testUrl'] = url if url else self.default_url
...
class BeautifulReport(ReportTestResult, PATH):
    ...
    def report(self, description, url, jenkins_branch, jenkins_time, filename: str = None, report_dir='.',
        ...
        self.stopTestRun(self.title, self.url, self.jenkins_time, self.jenkins_branch)
        if not report_flag:
